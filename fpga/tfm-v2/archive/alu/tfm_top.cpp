...
void parallel_fft(const real_t batch_fft_in[TIME_BATCH][N_COMBS], 
        ...
        for(int k = 0; k < N_COMBS; k += m){
            ...
            for(int j = 0; j < m / 2; ++j){
                ...
                for(int z = 0; z < TIME_BATCH; ++z){
                    #pragma HLS unroll
                    batch_fft_real_buffer[z][k] += cos_angle[k] * batch_fft_in[z][k];
                    batch_fft_imag_buffer[z][k] += sin_angle[k] * batch_fft_in[z][k];
                }
            }
        }
    }
    for(int k = 0; k < N_COMBS; ++k){
        ...
        for(int z = 0; z < TIME_BATCH; ++z){
            #pragma HLS unroll
            batch_fft_real[z][k] = batch_fft_real_buffer[z][k];
            batch_fft_imag[z][k] = batch_fft_imag_buffer[z][k];
        }
    }
}

void parallel_ifft(const real_t batch_fft_real_in[TIME_BATCH][N_COMBS], 
    ...
    for(int s = 1; s <= LOG_COMB; ++s){
        ...
        for(int k = 0; k < N_COMBS; k += m){
            ...
            for(int j = 0; j < m / 2; ++j){
                ...
                for(int z = 0; z < TIME_BATCH; ++z){
                    #pragma HLS unroll
                    batch_fft_real_buffer[z][k] += cos_angle[k] * batch_fft_real_in[z][k];
                    batch_fft_imag_buffer[z][k] += sin_angle[k] * batch_fft_imag_in[z][k];
                }
            }
        }
    }
...
void sa_dpp(const io_t* data, io_t* fft_write){
    ...
    for(int idx = 0; idx < N_TIMES / TIME_BATCH; ++idx){
        ...
        parallel_fft(batch_fft_in, batch_fft_real, batch_fft_imag);
        parallel_ifft(batch_fft_real, batch_fft_imag, batch_fft_real_out, batch_fft_imag_out);
...
extern "C"{
    void tfm(const io_t* data, io_t* fft_write, const io_t* fft_read, io_t* image_in, io_t* image_out, io_t* config, const int combs, const int dims, const int pixels){
        ...
        #pragma HLS interface m_axi port=fft_write offset=slave bundle=gmem3
